...
class ExampleSpider(scrapy.Spider):
    ...
    search_url = '''http://apps.webofknowledge.com/Search.do?product=UA&SID=3CId4cixczqg3aBLBkj&search_mode=GeneralSearch&prID=076982f2-0abb-49c6-8bc6-a67f8745b991'''
    ...
    def parse_search(self, response):
        ...
        if record_table:
            search_results = record_table.find("div", class_="search-results")
            if search_results:
                for record in search_results.find_all("div", class_="search-results-content"):
        ...
        # print(nextlink)
        if nextlink:
            yield scrapy.Request(nextlink, callback=self.parse_search,
                                 meta={'driver': self.driver, 'PhantomJS': True}, dont_filter=True)
    ...
    def parse_paper(self, response):
        ...
        for author in soup.find_all("a", title="Find more records by this author"):
            authors_list += author.get_text().strip() + ";"
